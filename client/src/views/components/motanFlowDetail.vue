...
export default {
  ...
  data() {
    return {
      ...
      httpFlag: false,
      otherFlag: false,
      activeResultTab:'subCallComparisonResult',
      stackTraces: '',
      paramCollapse: [],
      responseCollapse: []
    }
  ...
  mounted() {
    this.getDetail(this.traceId, this.taskRunId);
  },
  computed: {
    filterData() {
  ...
  methods: {
    ...
    getDetail(traceId, taskRunId) {
      if (traceId == '' && taskRunId == '') return
      API.record.flowDetail({ traceId, taskRunId }).then((res) => {
        if (res.code === 'SUCCESS') {
          ...
          this.request = this.entranceInvocation.request
          this.parameterTypes = this.entranceInvocation.parameterTypes;
          for (let i = 0; i < this.request.length; i++) {
            this.requestParameterArray.push({
              parameterValue: JSON.stringify(this.request[i]),
              parameterType: this.parameterTypes[i]
            })
          }
          if (this.entranceInvocation.response != null) {
    ...
    sonDetail(row) {
      ...
      if (row.type.toLowerCase() === 'http' || row.type.toLowerCase() === 'okhttp' || row.type.toLowerCase() === 'apache-http-client') {
        ...
        if (this.dialogRequest && this.dialogRequest.length) {
