...
public class ApacheHttpClientProcessor extends DefaultInvocationProcessor {
    ...
    public Identity assembleIdentity(BeforeEvent event) {
        ...
        try {
            ...
            Object uri = getRequestUri(request);
            if (uri instanceof String) {
                return new Identity(InvokeType.APACHE_HTTP_CLIENT.name(), (String) uri, "", null);
            }
            String path = (String) MethodUtils.invokeMethod(uri, true, "getPath");
            urlStr = HttpUtil.getPureUrL(MethodUtils.invokeMethod(uri, true, "getScheme"),
                    MethodUtils.invokeMethod(uri, true, "getHost")) + path;
        } catch (Exception e) {
            MoonboxLogUtils.error("apacheHttpClient-plugin assembleMockResponse error, event={}",
    ...
    public Object[] assembleRequest(BeforeEvent event) {
        ...
        try {
            ...
            Object request = event.argumentArray[1];
            // org.apache.http.RequestLine
            Object requestLine = MethodUtils.invokeMethod(request, true, "getRequestLine");
            Object uri = getRequestUri(request);
            Object queryParam = null;
            if (uri instanceof URI) {
                queryParam = MethodUtils.invokeMethod(uri, true, "getQuery");
            }
            Object method = MethodUtils.invokeMethod(requestLine, true, "getMethod");
            ...
            MoonboxLogUtils.error("apacheHttpClient-plugin assembleRequest error, event={}",
                    event.javaClassName + "|" + event.javaMethodName, e);
        }
    ...
    private String getRequestHeadersStr(Object request) throws Exception {
        ...
        String headersStr = "";
        Object[] headers = (Object[]) MethodUtils.invokeMethod(request, true, "getAllHeaders");
        if (headers == null) {
            return headersStr;
        }
    ...
    /**
     * 如果是BasicHttpEntityEnclosingRequest
     */
    private Object getRequestUri(Object request) throws Exception {
        if (request.getClass().getCanonicalName().contains("BasicHttpEntityEnclosingRequest")) {
            return FieldUtils.readField(request, "uri", true);
        }
        return MethodUtils.invokeMethod(request, true, "getURI");
    }

    private String getRequestBody(Object request, ClassLoader classLoader) throws Exception {

        Object httpEntity = MethodUtils.invokeMethod(request, true, "getEntity");
        if (httpEntity == null) {
            return StringUtils.EMPTY;
        }

        Long contentLengthObject = (Long) MethodUtils.invokeMethod(httpEntity, true, "getContentLength");
        if (contentLengthObject == 0) {
            return StringUtils.EMPTY;
        }

        // 保险起见在可重复读的时候才去获取body体内容
        Boolean isRepeatable = (Boolean) MethodUtils.invokeMethod(httpEntity, true, "isRepeatable");
        if (!isRepeatable) {
            MoonboxLogUtils.warn("apacheHttpClient-plugin request body is not repeatable");
            return this.DEFAULT_REQUEST_BODY_STR;
        }

        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        MethodUtils.invokeMethod(httpEntity, true, "writeTo", baos);
    ...
    private Charset determineCharset(Object httpEntity, ClassLoader classLoader) throws Exception {
        ...
        Object charset = MethodUtils.invokeMethod(contentType, true, "getCharset");
        if (charset == null) {
            return StandardCharsets.UTF_8;
        }
    ...
    public Object assembleResponse(Event event) {
        ...
        if (event.type == Event.Type.RETURN) {
            ...
            try {
                Object statusLine = MethodUtils.invokeMethod(response, true, "getStatusLine");
                Object statusCode = MethodUtils.invokeMethod(statusLine, true, "getStatusCode");
                Object reasonPhrase = MethodUtils.invokeMethod(statusLine, true, "getReasonPhrase");

                Object protocolVersion = MethodUtils.invokeMethod(statusLine, true, "getProtocolVersion");
                Object protocol = MethodUtils.invokeMethod(protocolVersion, true, "getProtocol");
                Object major = MethodUtils.invokeMethod(protocolVersion, true, "getMajor");
                Object minor = MethodUtils.invokeMethod(protocolVersion, true, "getMinor");
                ...
                MoonboxLogUtils.error("apacheHttpClient-plugin copy response error", e);
            }
        }
    ...
    private Map<String, List<String>> getResponseHeadersMap(Object response) throws Exception {
        ...
        Object[] headers = (Object[]) MethodUtils.invokeMethod(response, true, "getAllHeaders");
        ...
        for (Object header : headers) {
            String name = (String) MethodUtils.invokeMethod(header, true, "getName");
            String value = (String) MethodUtils.invokeMethod(header, true, "getValue");
    ...
    private String getResponseBody(Object response, ClassLoader classLoader) throws Exception {
        ...
        Object httpEntity = MethodUtils.invokeMethod(response, true, "getEntity");
        if (httpEntity == null) {
            return StringUtils.EMPTY;
        }

        Long contentLength = (Long) MethodUtils.invokeMethod(httpEntity, true, "getContentLength");
        if (contentLength == 0) {
            return StringUtils.EMPTY;
        }
    ...
    // todo 这里对请求的流进行了重新复制，不同场景的可靠性需要验证
    private void reSetResponseStream(ClassLoader classLoader, Object httpEntity, String responseBodyStr, Boolean gizp) throws Exception {
        ...
        if (gizp) {
            MethodUtils.invokeMethod(wrappedEntity, true, "setContent", this.getGzipByte(responseBodyStr, classLoader, httpEntity));
        } else {
            MethodUtils.invokeMethod(wrappedEntity, true, "setContent", new ByteArrayInputStream(responseBodyStr.getBytes()));
        }
    }
    ...
    private void addMockResponseHeaders(Object response, Map<String, Object> headersMap) throws Exception {
        ...
        for (Map.Entry<String, Object> header : headersMap.entrySet()) {
            ...
            for (String value : values) {
                MethodUtils.invokeMethod(response, true, "addHeader", name, value);
            }
        }
    }
    ...
    private void addMockResponseBody(Object response, String responseBody, ClassLoader classLoader) throws Exception {
        ...
        MethodUtils.invokeMethod(httpEntity, true, "setContent", inputStream);
        MethodUtils.invokeMethod(httpEntity, true, "setContentLength", bytes.length);
        MethodUtils.invokeMethod(response, true, "setEntity", httpEntity);
    }
