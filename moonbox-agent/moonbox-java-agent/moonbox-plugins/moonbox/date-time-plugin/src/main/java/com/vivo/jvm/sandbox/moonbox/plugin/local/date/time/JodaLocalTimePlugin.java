...
public class JodaLocalTimePlugin extends AbstractInvokePluginAdapter {
    /**
    ...
    protected List<EnhanceModel> getEnhanceModels() {
        ...
        EnhanceModel em = EnhanceModel.builder()
                .classPattern("org.joda.time.LocalTime")
    ...
    @Override
    protected JodaLocalTimeListener getEventListener(InvocationListener listener) {
        return new JodaLocalTimeListener(getType(), isEntrance(), listener, getInvocationProcessor());
    }
    ...
    public static class JodaLocalTimeListener extends DefaultEventListener {

        public JodaLocalTimeListener(InvokeType invokeType, boolean entrance, InvocationListener listener, InvocationProcessor processor) {
        ...
        protected boolean access(Event event) throws Exception {
            if (MoonboxContext.getInstance().isRepeatMode() && event instanceof BeforeEvent && MoonboxRepeatCache.isRepeatFlow()) {
                ...
                LocalTime dateTime = new LocalTime(time);
