 ...
 *
 * @author zhaoyb1990
 * Modifications Copyright 2022 vivo Communication Technology Co., Ltd. [修复isSandboxLoadedClass NPE]
 */
@Slf4j
public class ClassloaderBridge {
    ...
    public ClassLoader decode(String token) {
        ...
        if (loader == null) {
            if(token.equals("org.apache.catalina.loader.ParallelWebappClassLoader") && clsCached.get("org.apache.catalina.loader.WebappClassLoader") !=null){
                return clsCached.get("org.apache.catalina.loader.WebappClassLoader");
            }
            cacheClassLoader();
        }
    ...
    public List<Class<?>> findClassInstances(String className) {
        ...
        if(classes.size() >0){
            Set<Class<?>>uniqClasses =new HashSet<>(classes);
            return new ArrayList<>(uniqClasses);
        }
        return classes;
    }
     ...
     * @return true / false
     */
    public static boolean isSandboxLoadedClass(Class<?> clazz) {
        if (clazz == null) {
            return false;
        }
